...
if user_menu=='Medal Tally':
    ...
    if selected_year == 'Overall' and selected_country != 'Overall':
        st.title(str(selected_country) + ' Overall Perfomance')
    if selected_year != 'Overall' and selected_country != 'Overall':
        st.title(str(selected_country) + ' Perfomance in ' + str(selected_year) + ' Olympics')
...
if user_menu=='Athlete-wise Analysis':
    ...
    famous_sports = ['Basketball', 'Judo', 'Football', 'Tug-Of-War', 'Athletics', 'Swimming', 'Badminton', 'Sailing',
                     ...
                     'Ice Hockey', 'Racquets', 'Motorboating', 'Croquet', 'Figure Skating', 'Jeu De Paume', 'Roque',
    ...
    for sport in famous_sports:
        ...
        if len(gold_ages) > 1 and np.var(gold_ages) > 0:
            x_new.append(gold_ages.tolist())
            name_new.append(sport)

    fig = ff.create_distplot(x_new, name_new, show_hist=False, show_rug=False)
    fig.update_layout(autosize=False, width=1000, height=600)
